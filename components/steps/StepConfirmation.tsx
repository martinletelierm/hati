  ...
  return (
    <div className="text-center space-y-8 step-enter">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 divide-y divide-gray-50 text-left">
        ...
        <Row label="Envío a"      value={`${data.direccion}${data.comuna ? `, ${data.comuna}` : ''}`} />
      </div>

      <div className="bg-amber-50 rounded-xl px-5 py-4 text-sm text-amber-800 text-left">
        Te contactamos al <strong>{data.telefono}</strong> por WhatsApp en las próximas 24 h para confirmar y coordinar el envío.
      </div>
...
function Row({ label, value, mono, bold }: { label: string; value: string; mono?: boolean; bold?: boolean }) {
  return (
    <div className="flex items-start justify-between px-5 py-4 gap-4">
      ...
      <span className={`text-sm text-right ${mono ? 'font-mono' : ''} ${bold ? 'font-bold text-forest' : 'text-gray-700'}`}>
        {value}
      </span>
    </div>
  )
